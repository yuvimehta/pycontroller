...
def cmd_stop(args):
    ...
    if func_name not in functions:
        ...
        print(
            f"Available functions: {', '.join(functions.keys()) if functions else 'none'}"
        )
        return 1
...
Examples:
  ...
  pycontroller reset                 # Reset all flags
    ...
    ps_parser = subparsers.add_parser(
        "ps", help="List all running decorated functions")
    ...
    resume_parser = subparsers.add_parser(
        "resume", help="Resume a paused function")
    resume_parser.add_argument("function",
                               help="Name of the function to resume")
    ...
    # stop-all command
    stop_all_parser = subparsers.add_parser(
        "stop-all", help="Stop all running functions"
    )
    stop_all_parser.set_defaults(func=cmd_stop_all)

    # resume-all command
    resume_all_parser = subparsers.add_parser(
        "resume-all", help="Resume all paused functions"
    )
    resume_all_parser.set_defaults(func=cmd_resume_all)

    # kill-all command
    kill_all_parser = subparsers.add_parser(
        "kill-all", help="Kill all running functions"
    )
    kill_all_parser.set_defaults(func=cmd_kill_all)

    # status command
    status_parser = subparsers.add_parser(
        "status", help="Show status of function(s)")
    status_parser.add_argument(
        "function", nargs="?", help="Name of the function (optional)"
    )
    status_parser.set_defaults(func=cmd_status)

    # reset command
    reset_parser = subparsers.add_parser(
        "reset", help="Reset all control flags")
